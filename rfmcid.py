...
arrays=np.load('/home/dmf/python/npz/olsephilo.npz')
for i in tuple(('oidol','cid','date','churchcid')):
    vars()[i]=np.array(arrays[i])

arrays=np.load('/home/dmf/python/npz/llsephilo.npz')
...
dt=np.dtype([ ('cid', np.int64 ), ('oidol',np.int64 ), ('date',np.int64 ), ('church',np.int64 ),  ('merch' , np.float64 ) , ('hms' , np.float64 ) , ('pro' , np.float64 ) , ('gft' , np.float64 ), ('fic' , np.float64 ), ('kds' , np.float64 ), ('bib' , np.float64 ), ('aca' , np.float64 ),  ('clv' , np.float64 ),  ('chu' , np.float64 ),  ('mfg' , np.float64 ),  ('mus' , np.float64 ),  ('sea' , np.float64 ),  ('spa' , np.float64 ),  ('vid' , np.float64 ),  ('rec' ,np.int64 ), ('freq' , np.int64), ('mon' , np.float64), ('hmsmon' ,np.float64), ('promon' ,np.float64), ('gftmon' ,np.float64),  ('ficmon' ,np.float64), ('kdsmon' ,np.float64), ('bibmon' ,np.float64), ('acamon' ,np.float64), ('clvmon' ,np.float64), ('chumon' ,np.float64), ('mfgmon' ,np.float64), ('musmon' ,np.float64), ('seamon' ,np.float64), ('spamon' ,np.float64), ('vidmon' ,np.float64), ('score',np.int64)])
rfm=rf.unstructured_to_structured ( np.array([ cid[i] , oidol[i] , date[i] , churchcid[i], merch[i], hms[i], pro[i], gft[i], fic[i], kds[i], bib[i], aca[i],  clv[i],  chu[i],  mfg[i],  mus[i],  sea[i],  spa[i],  vid[i],   np.repeat(0,cid[i].size), np.repeat(0,cid[i].size), np.repeat(0,cid[i].size), np.repeat(0,cid[i].size), np.repeat(0,cid[i].size), np.repeat(0,cid[i].size), np.repeat(0,cid[i].size), np.repeat(0,cid[i].size), np.repeat(0,cid[i].size), np.repeat(0,cid[i].size),  np.repeat(0,cid[i].size),  np.repeat(0,cid[i].size),  np.repeat(0,cid[i].size),  np.repeat(0,cid[i].size),  np.repeat(0,cid[i].size),  np.repeat(0,cid[i].size),  np.repeat(0,cid[i].size),  np.repeat(-1,cid[i].size)]).T, dtype=dt )
...
def rfmselect(arr,frqmin,frqmax):
    cidcount=np.unique(rfm['cid'],return_counts=True)
    cida=cidcount[0][np.where( (cidcount[1]>frqmin) & (cidcount[1]<=frqmax) )]
    rfmtemp=rfm.copy()[np.isin(rfm['cid'],cida)]
    return(rfmtemp)
...
def frqroll(arr,frqmax):
    for i in np.arange(frqmax):
        cid2=np.roll(arr['cid'],i)
        arr['freq'][arr['cid']==cid2]=i
...
lst1=[ (0,10), (10,100), (100,250), (250,500), (500,1000), (1000,5000), (5000,10000) ]
...
#test cids: 
print(rfmappend[np.where(rfmappend['cid']==26831937)])
print(rfmappend[np.where(rfmappend['cid']==25346732)])

#save rfm
import os
os.chdir('/home/dmf/python/npz/')
np.savez_compressed('rfmcid.npz',rfm=rfmappend)
