...
catholic=np.repeat(0,church.size)
church=rfn.append_fields(church,'CATHOLIC',catholic,usemask=False)
idx=np.char.find(church['BILLTO'],'CATHOLIC')
church['CATHOLIC'][np.where(idx>0)]=1
idx=np.char.find(church['BILLTO'],'CATH CH')
church['CATHOLIC'][np.where(idx>0)]=1
idx=np.char.startswith(church['BILLTO'],'ST.')
church['CATHOLIC'][np.where(idx>0)]=1
idx=np.char.startswith(church['BILLTO'],'ST ')
church['CATHOLIC'][np.where(idx>0)]=1

idx=np.char.find(church['BILLTO'],'LUTHERAN')
church['CATHOLIC'][np.where(idx>0)]=0
idx=np.char.find(church['BILLTO'],'UMC')
church['CATHOLIC'][np.where(idx>0)]=0
idx=np.char.find(church['BILLTO'],'METHODIST')
church['CATHOLIC'][np.where(idx>0)]=0
idx=np.char.find(church['BILLTO'],'REFORMED')
church['CATHOLIC'][np.where(idx>0)]=0
np.unique(church['CID']).size
np.unique(church['CID'][church['CATHOLIC']==1]).size
np.savetxt(church[church['CATHOLIC']==1]
